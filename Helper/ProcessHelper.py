...
class ProcessHelper:
    ...
    @staticmethod
    def analyse_frames(historic_array, frames):
        ...
        if len(frames) > 0:
            ...
            if len(historic_array) == 0:
                ...
                if datetime.timedelta(seconds=1) < diff:
                    del historic_array[0]
                    historic_array.append((frames[0].copy(), now))

            pool = []
            for frame in frames:
                pool.append(asyncio.ensure_future(ProcessHelper.get_numberplate(historic_array, frame), loop=loop))
            ...
            loop.close()
            return out, historic_array

        return None, None

    @staticmethod
    @asyncio.coroutine
    def get_numberplate(historic_frames, frame):
        ...
        if len(obj_roi) > 0:
            ...
            for target in targets:
                ...
                rectangles, boxes, angles = ContourHandler.get_rectangles(contours, tmp, area_bounds=(0.1, 10),
                                                                          min_point=(5, 5), max_point=(20, 20))
                if len(rectangles) > 0:
                    potential_plates = ImageUtil.char_roi(tmp, rectangles)
                    if potential_plates is not None:
                        cropped_array = ImageUtil.process_for_tess(image=tmp, rectangles=rectangles)
